...
urlpatterns = [
  ...
  re_path(r'^pay_principle(?:/(?P<pk>\d+))?/$', pay_principle, name="pay_principle"),
  re_path(r'^payment_summary(?:/(?P<pk>\d+))?/$', payment_summary, name="payment_summary"),
  re_path(r'^outward_list(?:/(?P<notification>[\w-]+))?/$', outward_view, name="outward_view"),
  re_path(r'^outward/(?P<pk>\d+)/$', outward_entry_view, name="outward_entry_view"),
