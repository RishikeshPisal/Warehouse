...
function addItem(that) {
  ...
  // if (items[product_id] + quantity > stock) {
  //   console.log('not enough');
  //   notEnoughBalanceToast()
  //   return
  // }
  if (!items[product_id]) {
...
// mId = document.getElementById('admin-id').value
// async function validateadminIdUsingFetch() {
//   const admin_id = document.getElementById('admin-id').value.trim()
//   const admin_name = document.getElementById('admin-name')
//   const admin_mobile_no = document.getElementById('admin-mobile-no')

//   const response = await fetch('/main/get_admin_details/', {
//     method: 'POST',
//     headers: {
//       'Content-Type': 'application/x-www-form-urlencoded',
//     },
//     body: `admin_id=${admin_id}`, // body data type must match "Content-Type" header
//   })
//     .then(response => {
//       if (response.ok) {
...
//         admin_mobile_no.value = data.mobile_no
  ...
  fetch(url, {
    ...
    .then(data => {
      ...
      if(data.success)
        ...
        // window.location = `/franchise/orderToAdmin/`;
        document.getElementById('order-invoices-link').click()
      else
...
// async function validateadminIdUsingAjax() {
//   const admin_id = document.getElementById('admin-id').value.trim()
//   validateadminIdUsingFetch(admin_id)
//   const admin_name = document.getElementById('admin-name')
//   const admin_mobile_no = document.getElementById('admin-mobile-no')
//   let flag = false
...
//   xhr.open('POST', '/main/get_admin_details/', true)
...
//       // Handle the response
//       if (response.name) {
//         admin_name.value = response.name
//         admin_mobile_no.value = response.mobile_no
//         flag = true
//         // return
//       } else {
//         flag = false
...
//   // xhr.send('admin_id=' + encodeURIComponent(admin_id))
//   return flag
// }
...
async function checkadminId() {
  const admin_id = document.getElementById('admin-id').value.trim()
  if (document.getElementById('admin-id').value) {
    document.getElementById('admin-id').disabled = true
    document.getElementById('check-id-btn').classList.add('d-none')
    document.getElementById("hidden-field-2").classList.remove('d-none')
    document.getElementById("hidden-field-3").classList.remove('d-none')
